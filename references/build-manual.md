...
**macOS / Linux**：
```bash
nohup python3 ~/myaiweb/scripts/registry_server.py > ~/registry.log 2>&1 &
```

**Windows**（后台、无窗口）：
```powershell
Start-Process pythonw -ArgumentList "C:\myaiweb\scripts\registry_server.py" -WindowStyle Hidden
```

验证（零依赖自测，**不需要 redis-cli**）：
```bash
python3 ~/myaiweb/scripts/registry_client.py 127.0.0.1   # 打印「✅ 裸 socket RESP 通了」即成
```
...
```bash
python3 ~/myaiweb/scripts/register_node.py --registry-host 127.0.0.1
```
...
```bash
python3 ~/myaiweb/scripts/dashboard.py --registry-host 127.0.0.1
```
...
macOS / Linux：
```bash
nohup python3 ~/myaiweb/scripts/dashboard.py --registry-host 127.0.0.1 > ~/dashboard.log 2>&1 &
echo "Dashboard PID: $!"
```

Windows（PowerShell）：
```powershell
Start-Process pythonw -ArgumentList "C:\myaiweb\scripts\dashboard.py --registry-host 127.0.0.1" -WindowStyle Hidden
