...
> - **存储**看 `hardware.disks` **全部盘**，**别用 `storage` 摘要**——那串只含系统盘 C:，多盘机会漏掉 D:/E: 的大盘（"没存储节点 / 盘都不大"这类误判就是这么来的）。**求和有讲究**：Windows/Linux 多物理盘各算各的、相加；**macOS 上 `disks` 是 APFS 同一容器的多个卷（`total`/`avail` 都相同），按容器去重、别叠加**（否则一块 228G 的盘会被算成 1.3T）。
