...
**没有明确指令时——开场就读全网、逐台摆事实 + 菜单（开场即评估，不再单设"评估"项）:**

开场**直接读注册中心每张卡的完整字段**、逐台把客观事实摆出来——只报事实 / 能力，不给建议（建议归 ③ 其他里的"优化"）。**下面这条"读全卡"纪律，就是开场必须做的**（以前它在单独的"评估"里，结果开场松散概括、把 OS / 盘报错——现在合并：开场即按此读，没有"先松散概括、再单独评估"两套）：

> **铁律：只读卡里的客观事实，别读子集、别拿记忆填空。**
...
> - **OS / CPU / GPU+VRAM / RAM** 读卡对应字段——**别猜版本**（卡里写 `Windows 10` 就别报成 Win11）。
> - **已装模型**看 `models`、**经验便签**看 `notes`、agent 看 `agents`——一个都不漏（`notes` 里常有别人实测的最优配置 / 踩过的死路，不读就会重复试错）。
> - "此刻在不在跑 / GPU 占用"卡里没有，要准现场 `nvidia-smi` / `ollama ps`；但**能力、盘、OS、已装模型、notes 都在卡里，必须读卡、不许猜**。

逐台摆出来 + 菜单（**不用 ✅/打勾矩阵；只说"能"、不说"该"——错配 / 闲置 / 该装啥归 ③ 优化**）：
...
- **③ 其他**（维护菜单，展开才列，平时不占地方）:
  - **优化** — **站在全网做分工规划，不是逐台填空**。拿开场读到的客观能力，把活**互补地分下去、不重复、补缺口**：
...
| "评估" / "优化网络" / "现在该装什么" | 读三层事实（硬件/环境/网速）+ 各卡 `problems` → 逐台具体 + 建议（闲置硬件 / 缺工具 / 错配 / 瓶颈；AI 能力分本地/云端两看）= 开场那条（开场即评估、把卡读全摆事实） |
...
**何时设（别滥设）**：要**持续 / 交互**地占着那台干（反复进去、文件长期留在 `work_dir`、当远程开发盒）才设。**一次性任务**（跑一条命令、一次训练）直接 `dispatch --node X "命令"` 就够，**不必设工作区**。**从不自动设** —— 触发只有两个：你明说「在 X 上开个工作区」，或控制模式开场（读全卡）发现「本机盘满 / 没 GPU + 某节点有富余」时主动提议。
...
**④b 本地把手（给 Desktop 的 Claude/codex 用）** —— 那些 app 开会话只能选/建**本地**文件夹当工作区、没法直接开远程工作区。做法：**用户在 Desktop 选定（或新建）一个本地文件夹当工作区、在里面开会话；你就在那个文件夹里跑**:
```bash
python3 ~/myainet/scripts/setup_workspace.py --handle <远程节点>   # md 默认落【当前目录】=用户选定的工作区
```
它把 `CLAUDE.md`/`AGENTS.md` **写进当前文件夹**（内容＝那台远程工作区的 OS 契约：节点/work_dir/os/shell/python/gpu/ssh + 怎么派活）。这样**用户选的文件夹里就有了 md** → agent 一打开自动读 → 知道真活在远端、去那边干（`dispatch --workspace` / `ssh`）。**本地只多这俩 md、不占地方。**（要落到别处用 `--at <路径>`。）
