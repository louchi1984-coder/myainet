...
> - **Wakeable (WoL)**: see `wake`. If a node is **offline but its card has `wake.mac`**, mark it as **wakeable** (it's asleep, not dead — you can wake it remotely via WoL with `wake.py`), don't lump it with truly-dead nodes.
