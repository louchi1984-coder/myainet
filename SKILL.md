...
> - **读卡的 `notes`**（agent 写的便签）：装了哪些**非-LLM 能力**(TTS/生图/数字人/向量库)、**实测出的最优配置**、别人**踩过的坑**——这些 `models`/`cli` 里没有、全在 `notes`。评估/派任务前先看，别无视已装能力、别重复别人验过为死路的方向（如"此卡 TTS 用 GPU 反而慢 3-5x"）。
...
- **④ 其他**（维护菜单，展开才列，平时不占地方）:
  - **优化** — **站在全网做分工规划，不是逐台填空**。拿②评估的客观能力，把活**互补地分下去、不重复、补缺口**：
    ...
    > **铁律：优化里所有具体方案（用哪个本地模型 / 生图 / 音频 / 数字人 / 向量库 / 框架 / 工具）一律 AI 联网搜当下最佳，禁用训练记忆里的名字。** 训练记忆早过时（`qwen2.5` 那种坑就是凭记忆报旧名）；先搜「当前 <VRAM>G 显存最佳 X」再给型号/版本。
    > **再一层：搜来的也只是通则——"这张卡能不能加速 / 能不能跑某模型"在那台机器上实测一遍才算数，把结论（最优配置 + 走不通的方向）`report --card` 钉进卡。** 真实教训：某 2070 上 IndexTTS2 用 GPU 反比 CPU 慢 3-5x（与"GPU 必快"的常识相反）——不实测、不写卡，下个任务还会再踩一遍。
...
>
> **铁律——装出/调出/试出"别人将来用得上的事实"，加 `--card` 钉到那台节点的卡上（不只写流水）：**
> `report.py --node <节点> --card "装了 IndexTTS2(TTS)；实测此卡 device=cpu 最优，GPU 在 2070 上慢 3-5x，别再试 GPU 加速"`。
> 卡里只有 `models`（LLM）这一类硬采字段——**非 LLM 的能力（TTS / 生图 SD·ComfyUI / 数字人 / 向量库）、实测出来的最优配置、踩过的反直觉坑，sysinfo 采不到，全靠你写进 `notes`**。**信任你的判断：什么是"别的任务该知道、否则会重复试错"的，就 `--card` 写进卡。** 写进去后 patrol 每小时重注册也冲不掉（`register_node` 自动带上已有 notes）。下个任务读卡的 `notes` 即知，不必从零再试。
