...
- `setup_workspace.py` — **(set a node as a native workspace)** control runs it remotely against the node: create `work_dir` on the chosen disk + write the self-report marker `~/.myainet/workspace.json` + trigger register to self-report the OS contract into the card (**no container, no Docker**); afterwards `dispatch --workspace` sends work in
