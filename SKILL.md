...
name: myaiweb
description: >
  myaiweb 个人 AI 网络搭建工具。当用户想了解某台机器在 AI 时代能承担什么任务、
  给机器打节点名片、或者把多台机器组成 myaiweb 个人 AI 网络时，必须使用此 skill。
...
把多台机器组成 myaiweb 个人 AI 网络——各司其职，任意 AI 工具加载网络配置后即可统一调度：

```
任意 AI 工具（加载 myaiweb 网络配置后）按任务匹配 4 类能力节点：
...
下文示例里的 **`~/myaiweb/scripts/...` 是简写，指「本 skill 包内的 `scripts/` 目录」**（和这份 SKILL.md 同级）。skill 实际装在哪个 agent 的 skills 目录，就用那个真实路径，例如：

- opencode：`~/.config/opencode/skills/myaiweb/scripts/`
- Claude Code：`~/.claude/skills/myaiweb/skills/myaiweb/scripts/`（以实际加载位置为准）
- 手动 clone：clone 到哪就是哪

**别假设 `~/myaiweb` 真实存在**——跑命令前把前缀替换成本 skill 的真实位置（你加载本文件时就知道它在哪）。机器上若残留旧拷贝，以 skill 真身为准，别跑到旧目录里执行过期脚本。
...
- **Windows**：`winget install -e --id Python.Python.3.12`；winget 不可用 → 下载 python.org 安装器静默安装；再不行 → 解压官方 **embeddable zip** 到 myaiweb 目录直接用（免管理员、不污染系统）。
...
```bash
python3 ~/myaiweb/scripts/identity.py
```
...
```bash
python3 ~/myaiweb/scripts/setup_hub.py --verify     # 逐项核：注册中心 / 大屏 / 巡检 / SSH / Tailscale
```
- `✅ … 全部就位` → 进**控制模式**（见下）。
- `❌ 没建完，还差：X`（退出码非 0）→ **先补建**：跑 `python3 ~/myaiweb/scripts/setup_hub.py`（幂等，只补缺的、不重起已在跑的），补到 ✅ 再进控制模式。重启后大屏 / 巡检没自启也走这条。

**② `role=主控`** → 它也控制全网、但不扛 infra。连标记里的 `central`（中央建网机）→ 进**控制模式**，拿到和建网机一模一样的控制台与命令。
> **新机器要配成主控**（还没身份标记 / 是台干净机器）→ 跑确定性脚本，别手敲：`python3 ~/myaiweb/scripts/setup_control.py --central <建网机地址>`（同 LAN 填它 lan_ip，异地填它 Tailscale IP）。它按顺序做完「Tailscale → 写身份(主控,central=建网机) → SSH → 注册自己 → 存本地镜像 → 自检」，每步自验、`--verify` 可只核不动手。**主控的 central 必须是建网机地址、绝不能自指**（填 127.0.0.1 会让裸加载 skill 查不到注册中心、误判「注册表空」——脚本会当场拒）。

**③ `role=次建网机`**（建网机的精简版：本地注册中心 + 同步给主，不起大屏/巡检）→ 核验/补起：`python3 ~/myaiweb/scripts/setup_hub.py --main <central> --verify`，没建完就去掉 `--verify` 跑一遍补上。它是本 LAN 的 infra（数据随同步汇进主的大屏）；要看全网 / 派活去主或主控。
...
如果建网机上已启动 Dashboard，提示用户直接打开浏览器查看实时状态（地址从上次启动日志获取，或提示运行 `python3 ~/myaiweb/scripts/dashboard.py`）。
...
- **② 评估** — 逐台落到**具体事实**（在线 + 真规格 + 装了啥 + 待解决）+ 指出闲置 / 缺工具 / 错配 / 瓶颈。**看状况、出诊断（不动手）**。**不用 ✅/推理 这种打勾矩阵**（打勾正是 facts-only 反对的"分类"）:

```
🧱 win-desktop      建网机  ✅ 在线  Win11·i5·RTX3060 12G·16G·SSD 50/200G
     装了:ollama✗ agent✗   ⚠️ 有 3060 没装 ollama → 本地大模型跑不起来（装上可跑 7B/14B-Q4）
💻 mac-laptop       主控    ✅ 在线  M1·8G·SSD 14.8/228G   装了:claude✓ codex✓
     ⚠️ 盘 14.8/228G 快满
💾 nas-box          节点    ❌ 离线 2 天 → 退网还是等?
```

那行 `⚠️` 来自卡的 `problems` + 闲置 / 缺工具 / 盘满 / 持续离线。

- **③ 任务** — 进「任务拆解与路由」（见下）:拆解 → 匹配 → `dispatch`。
- **④ 其他**（维护菜单，展开才列，平时不占地方）:
  - **优化** — 照 ② 评估的诊断**动手改**（装 ollama / 清盘 / 装 agent…），多半落到 `dispatch`
...
**每次读注册表，顺手镜像一份到本地**：`python3 ~/myaiweb/scripts/registry_cache.py --registry-host <central>` —— 这样**建网机掉线时主控仍知道每台机器**，`dispatch` 会自动回退这份镜像、直驱够得到的机器（同 LAN 直连不经建网机）；这份镜像也是**转移时的注册表备份**。
...
>
> **铁律：判节点死活 = `python3 ~/myaiweb/scripts/dispatch.py --node <名> --check`，绝不自己跑 `ping`。** ping 走 ICMP，**Windows 节点防火墙默认就拦 ICMP**——ping 100% 丢包是常态，跟机器死活无关。只凭 ping 判死活会把活得好好的机器误判成「硬 down」，进而触发无谓的「恢复/转移」（真实事故反复发生）。`--check` 走的是你真正控制它的那条 SSH 路（LAN→Tailscale→建网机跳板），连上=活、连不上才是真够不到。**看到自己想敲 `ping <节点>` 判在线，就停手，改 `--check`。**

> **配套——改了机器状态就 `report` 一笔：** 你（或你派的 agent）装/删软件、换版本、回滚、"试了又退"这类**会改变机器状态**的动作，边做边报一句：
> `python3 ~/myaiweb/scripts/report.py --registry-host <主IP> --node <节点> "在装新 ollama / 不行已回退"`（拿不准 / 出岔子加 `--warn`）。
...
```bash
python3 ~/myaiweb/scripts/dispatch.py --registry-host <主IP> --node <节点> --name <任务名> "<命令>"
```
...
**够不到节点就穿它的归属建网机**（节点在别的 LAN，或你在外面够不到家里）：看节点卡的 `belongs_to` 字段——**别直连，而是 `ssh 那台建网机`、在它上面跑 dispatch**（本地 LAN 节点的建网机=主建网机；别的 LAN=那台次）。在建网机上跑时按 **它卡里的 `os`** 出命令：posix `python3 ~/myaiweb/scripts/dispatch.py …`，Windows `python C:\myaiweb\scripts\dispatch.py …`。够得到就直连、别多绕。统一模型 **主控 → 节点的建网机 → 节点**，没有"外地"特殊——`belongs_to` 就是这把通用路由键。
...
macOS / Linux：
```bash
python3 ~/myaiweb/scripts/setup_hub.py
```
...
PowerShell：
```powershell
chcp 65001 > $null; [Console]::OutputEncoding = [System.Text.Encoding]::UTF8; python $env:USERPROFILE\myaiweb\scripts\setup_hub.py
```

cmd：
```cmd
chcp 65001 >nul && python %USERPROFILE%\myaiweb\scripts\setup_hub.py
```
...
**一个收尾（setup_hub 不做，手动跑一次）**：**Tailscale 代理绕过**（防 `100.x` 被代理吞成 502）：`python3 ~/myaiweb/scripts/tailscale_proxy_bypass.py`（用 Clash/Surge 还要在其规则里加 `100.64.0.0/10 DIRECT`）。
...
python3 ~/myaiweb/scripts/setup_hub.py --main <主的Tailscale地址>
```
...
```bash
# 按进程名匹配，最省事（不用记 pid）
python3 ~/myaiweb/scripts/watch_job.py --registry-host <主建网机IP> --node <节点名/IP> --name 夜间训练 --match "train.py"
# 或按 pid 精确盯
python3 ~/myaiweb/scripts/watch_job.py --registry-host <主建网机IP> --node mac-studio --name 渲染 --pid 4321
python3 ~/myaiweb/scripts/watch_job.py --registry-host <主建网机IP> --list             # 看登记了哪些
python3 ~/myaiweb/scripts/watch_job.py --registry-host <主建网机IP> --unwatch 夜间训练   # 撤
```
...
nohup python3 ~/myaiweb/scripts/patrol.py --registry-host 127.0.0.1 > ~/patrol.log 2>&1 &
```
...
- **顺 myaiweb 的纹理** —— 网本就异构、`dispatch` 本就按节点 `os` 出命令。代价：工作区是**那台的原生环境**（Win 给你 PowerShell、不是 bash）—— 所以下面 ③ 那条「读契约 + 走 dispatch」是硬前提，agent 才不跨系统出错。
...
```bash
ssh <节点> "<卡里的 python> ~/myaiweb/scripts/setup_workspace.py \
...
- **派活**：一律 `dispatch --workspace`，它读卡 `work_dir`、**按 `os` 自动 cd**（agent 不手写跨 OS 路径）：
  ...
  python3 ~/myaiweb/scripts/dispatch.py --node <节点> --workspace --name 训练 "<workspace.python> train.py"
  ```
...
如果适合跑大模型，同时评估是否适合**训练/微调**：VRAM ≥ 16GB → 可以跑 LoRA/QLoRA，推荐 Unsloth（`pip install unsloth`）；VRAM ≥ 40GB → 可全量微调。
...
**可用空间充足（可用 > 总量 30%）**：
- 总量 ≥ 1TB → 推荐作为存储节点，部署：
  - MinIO（S3 兼容对象存储，存模型权重/数据集）：`docker run -d -p 9000:9000 -p 9001:9001 minio/minio server /data --console-address ":9001"`
  - Qdrant（个人向量数据库，RAG 知识库）：`docker run -d -p 6333:6333 qdrant/qdrant`
...
- **自动化编排**：安装 n8n → `docker run -d -p 5678:5678 n8nio/n8n`，或 Python cron 脚本
...
3. **在 myaiweb 网络中建议承担的角色**（可以多个）

示例输出：
```
这台机器（gpu-rig）：
✅ 现在就能做：本地大模型推理（VRAM 16GB）
   → 联网查到当前推荐：Qwen2.5-14B-Instruct Q4_K_M
   → 安装：ollama pull qwen2.5:14b
...
注册成功后输出：`✅ 节点 <hostname> 已注册到 myaiweb（长期保存）`，并打印 `🔑 换钥匙：装入 N 把控制方公钥`。
...
### 💾 存储
<可用空间，是否推荐作存储节点，推荐部署 MinIO/Qdrant 或给出清理建议>

### ⚙️ 脚本 / 自动化 / 测试
<推荐 n8n / Python cron / 测试框架，适合哪类任务>
...
主控用来查看整个 myaiweb 网络状态，生成可加载进任意 AI 工具的网络调度配置。
...
```bash
python3 ~/myaiweb/scripts/register_node.py --role 主控
...
**优化建议——只提「facts 一摆就成立」的，别瞎猜；每条给出口（具体命令/步骤）：**

- **闲置硬件**：有 GPU 却没装 ollama/agent → "装上就能解锁本地/云端 AI"（一块 12G 显卡空着是最典型的浪费）；
- **缺关键工具**：想当某角色却缺运行时（要跑 python 项目却没 python）→ 提装；
- **错配 / 浪费**：强机当纯节点闲着，或弱机硬扛重活（8G 机别派本地大模型，给它派云端）；
- **瓶颈**：节点在蜂窝 / 盘快满 / RAM 紧 → 提醒（如"主控盘只剩 14G，数据往盘空的那台放"）。
...
你现在接入了一套 myaiweb 个人 AI 网络。接到任务：① 按类型选合适的节点（本地能干的先本地，再云端 API）；② **经 dispatch 执行、会改机器状态的动作 report 一笔** —— 别自己 ssh 直连跑活儿，那样网络对它就失去感知了。

【建网机（中枢）】<hostname>  LAN <LAN_IP> / TS <TS_IP>（外网用 TS）
- 注册中心 注册中心：<建网IP>:27182    脚本：~/myaiweb/scripts/
...
1. **在节点上跑任何活儿 = 经 dispatch，不要自己 ssh "命令" 直连：**
   python3 ~/myaiweb/scripts/dispatch.py --registry-host <建网IP> --node <节点> --name <任务名> "<命令>"
...
2. **会改变机器状态的动作，边做边 report**（装/删软件、换版本、回滚、试了又退…）：
   python3 ~/myaiweb/scripts/report.py --registry-host <建网IP> --node <节点> "在装新 ollama / 不行已回退"
...
**核心 / 共用**
- `registry_server.py` — **零依赖注册中心**（顶替 Valkey/Redis）：sqlite + 标准库 RESP server，监听 27182。建网机跑这一个 python 进程即可，**三系统原生、免装、无 Docker/WSL**；卡落 `~/.myaiweb/registry.db`（WAL，崩了/重启不丢），只实现 myaiweb 用到的 SET(+EX)/GET/MGET/KEYS/DEL
