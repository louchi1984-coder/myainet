...
> **铁律：读完整卡、别读子集，更别拿训练记忆填空。** 评估里的每个事实都从卡的对应字段读——
> 大屏能显示的，卡里就有、你也读得到（同一个注册中心）：
> - **存储**看 `hardware.disks` **全部盘**（按各盘 `avail_gb`/`total_gb` 求和），**别用 `storage` 摘要**——那串只含系统盘 C:，多盘机会漏掉 D:/E: 的大盘（"没存储节点 / 盘都不大"这类误判就是这么来的）。
> - **本地大模型**看卡的 `models` 字段（装了哪些、可不可用），**别断言"没拉模型"**——装没装、装了啥，以 `models` 为准。
> - **别报训练记忆里的模型名/版本**（如 qwen2.5——多半已过时，卡里实装的往往更新）。要荐新模型就联网查当下的、按显存给型号；装了啥一律以卡为准。
> - "此刻在不在跑 / GPU 占用"这种**现状**卡里没有，要准可现场 `nvidia-smi` / `ollama ps`；但**盘、已装模型这种事实卡里有，必须读卡，不许猜**。
...
读卡的 `hardware.disks` **全部盘**（每块 `{mount, total_gb, avail_gb}`）算存储——**总量＝各盘之和、最大空闲盘＝存储落点**；**别只看 `storage` 摘要**（那只含系统盘 C:，多盘机会把 D:/E: 的大盘漏掉，误判成"盘小 / 没存储节点"）：
